...
class NonSASL(PlugIn):
    ...
    def plugin(self,owner):
        ...
        if query.getTag('digest'):
            ...
            hasher = hashlib.sha1()
            hasher.update(owner.Dispatcher.Stream._document_attrs['id']+self.password)
            query.setTagData('digest', hasher.hexdigest())
            ...
            hasher1.update(self.password)
            hasher2.update(hasher1.hexdigest()+token)
            hashed = hash2.hexdigest()
            for foo in xrange(int(seq)):
                hasher3 = hashlib.sha1()
                hasher3.update(hashed)
                hashed = hasher3.hexdigest()
            query.setTagData('hash',hashed)
            method='0k'
        else:
    ...
    def authComponent(self,owner):
        ...
        hasher = hashlib.sha1()
        hasher.update(owner.Dispatcher.Stream._document_attrs['id']+self.password)
        owner.send(Node(NS_COMPONENT_ACCEPT+' handshake',payload=[hasher.hexdigest()]))
